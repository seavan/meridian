...
namespace meridian.diagram
{
    public class ClassFile : CodeFile
    {
        public override void WriteInners(CSharpWriter _writer)
        {
            ...
            {
                _writer.WriteIndent("[MetadataType(typeof({0}))]", MetadataType);
            }
            _writer.WriteClass(ClassName + (!string.IsNullOrEmpty(Inherits) ? " : " + Inherits : ""));
            _writer.OpenBlock();
        ...
        public virtual void Classes(CSharpWriter _writer)
        {
        ...
        {
            var reserved = new string[] { "event", "delegate", "class", "long" }.ToList();
