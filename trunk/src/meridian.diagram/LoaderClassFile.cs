...
namespace meridian.diagram
{
    public class LoaderClassFile : ClassFile
    {
        ...
        public override void Classes(CSharpWriter _writer)
        {
            base.Classes(_writer);

            using (_writer.Block("public interface IEntityStore", ClassName))
            {
                _writer.WriteIndentEol("object GetObject(long _id)");
                _writer.WriteIndentEol("Type GetObjectType()");
                _writer.WriteIndentEol("void DeleteById(long _id)");
                _writer.WriteIndentEol("void UpdateById(long _id)");
            }
        }
        ...
        public override void Functions(CSharpWriter _writer)
        {
            ...
            _writer.WriteIndentEol("var now = new DateTime()");
            foreach (var proto in protos)
            {
                _writer.WriteIndentEol("Tracer.I.Debug(\"initialized {0} store\")", proto.Name);
                _writer.WriteIndentEol("{0}Store = new protoStore.{0}Store()", proto.Name);
                _writer.WriteIndentEol("m_Stores[\"{0}\"] = {0}Store", proto.Name);
                _writer.WriteIndentEol("now = DateTime.Now");
                if (!proto.SkipLoad)
                {
                ...
                {
                    _writer.WriteIndentEol("Tracer.I.Debug(\"skip loading {0} store\")", proto.Name);
                }
                _writer.WriteIndentEol("m_LoadingTimes[\"{0} select\"] = (DateTime.Now - now).TotalMilliseconds", proto.Name);
                _writer.WriteIndentEol("Tracer.I.Debug(\"loaded {0} store, estimated {{1}}\", m_LoadingTimes[\"{0} select\"])", proto.Name);
            }
            ...
            foreach (var proto in protos)
            {
                ...
                _writer.WriteIndentEol("m_LoadingTimes[\"{0} aggregations\"] = (DateTime.Now - now).TotalMilliseconds", proto.Name);
                if ((proto.Compositions.Count() > 0) || (proto.InlineCompositions.Count() > 0))
                {
                    ...
                    _writer.WriteIndentEol("m_LoadingTimes[\"{0} compositions\"] = (DateTime.Now - now).TotalMilliseconds", proto.Name);
