...
namespace meridian.diagram
{
    public class ProtoStoreClassFile : ClassFile
    {
        public ProtoStoreClassFile(ProtoDescription _descr)
        {
            ...
            Inherits = String.Format("Meridian.IEntityStore, admin.db.IDataService<proto.{0}>", _descr.Name);
            ProtoClassName = "proto." + _descr.Name;
            m_Description = _descr;
            if (_descr.Fields.Count > 0)
            {
                if (!String.IsNullOrEmpty(_descr.CustomKeyName))
                {
                    ProtoPrimaryIdName = "id";
                    ProtoPrimaryIdType = FieldType.ftLong;
                    ;
                }
                else if(_descr.Fields.Count(s => s.Name.ToLower().Equals("id")) > 0)
                {
                    ProtoPrimaryIdName = "id";
                    ProtoPrimaryIdType = _descr.Fields.First(s => s.Name.Equals(ProtoPrimaryIdName)).FieldType;
                    ;
                }
                ...
                else
                {
                    ...
                    ProtoPrimaryIdType = _descr.Fields.First().FieldType;
                }
        ...
        public FieldType ProtoPrimaryIdType { get; set; }
 ...
 */
            ...
            using (_writer.Block("public void Commit()"))
            {
                _writer.WriteIndentEol("//throw new NotImplementedException()");
            }

            /*

        public IQueryable<restaurants> GetAll()
        {
            return Queryable.AsQueryable<restaurants>(All());
        }
             */

            using (_writer.Block("public IQueryable<proto.{0}> GetAll()", m_Description.Name))
            {
                _writer.WriteIndentEol("return Queryable.AsQueryable<proto.{0}>(All())", m_Description.Name);
            }
        ...
        public restaurants GetById(long id)
        {
            ...
            using (_writer.Block("public proto.{0} GetById(long id)", m_Description.Name))
            {
                _writer.WriteIndentEol("return Get(id)", m_Description.Name);
            }
            /*
        void admin.db.IDataService<restaurants>.Insert(restaurants item)
        {
            Insert(item);
        } */
            if(!m_Description.CustomDataServiceInsert)
            using (_writer.Block("void admin.db.IDataService<proto.{0}>.Insert(proto.{0} item)", m_Description.Name))
            {
                _writer.WriteIndentEol("Insert(item)", m_Description.Name);
            }
            /*
        void admin.db.IDataService<restaurants>.Delete(restaurants item)
        {
            Delete(item);
        } */
            if (!m_Description.CustomDataServiceDelete)
            using (_writer.Block("void admin.db.IDataService<proto.{0}>.Delete(proto.{0} item)", m_Description.Name))
            {
                _writer.WriteIndentEol("Delete(item)", m_Description.Name);
            }
            /*
        public restaurants CreateItem()
        {
            ...
            if (!m_Description.CustomDataServiceCreate)
            using (_writer.Block("public proto.{0} CreateItem()", m_Description.Name))
            {
                _writer.WriteIndentEol("return new proto.{0}() {{   }}", m_Description.Name);
            }

            /*
        public void AbortItem(restaurants item)
        {
            Delete(item);
        } */
            using (_writer.Block("public void AbortItem(proto.{0} item)", m_Description.Name))
            {
                _writer.WriteIndentEol("Delete(item)", m_Description.Name);
            }
 ...
 * 
            */
            using (_writer.Block("public void Discard()", m_Description.Name))
            {
                _writer.WriteIndentEol("", m_Description.Name);
            }
            /*
        void admin.db.IDataService<restaurants>.Update(restaurants item)
        {
            Persist(item);

        }
 */
            if (!m_Description.CustomDataServiceUpdate)
            using (_writer.Block("void admin.db.IDataService<proto.{0}>.Update(proto.{0} item)", m_Description.Name))
            {
                _writer.WriteIndentEol("Persist(item)", m_Description.Name);
            }

            // -------------


            using (_writer.Block("public object GetObject(long _id)"))
            {
                _writer.WriteIndentEol("return Get(_id)");
            }

            using (_writer.Block("public System.Type GetObjectType()"))
            {
                _writer.WriteIndentEol("return typeof({0})", ProtoClassName);
            }

            using (_writer.Block("public void DeleteById(long _id)", m_Description.Name))
            {
                _writer.WriteIndentEol("Delete(Get(_id))", m_Description.Name);
            }

            using (_writer.Block("public void UpdateById(long _id)", m_Description.Name))
            {
                _writer.WriteIndentEol("Update(Get(_id))", m_Description.Name);
            }
            ...
            using (_writer.Block("public {0} Insert({1}Connection _connection, {0} _item)", ProtoClassName, this.AdoNetPrefix))
            {
                ...
                {
                    switch (ProtoPrimaryIdType)
                    {
                        case FieldType.ftInt:
                            _writer.WriteIndentEol("_item.id = Convert.ToInt32(cmd.ExecuteScalar())");
                            break;
                        default:
            ...
            using (_writer.Block("public {0} Update({1}Connection _connection, {0} _item)", ProtoClassName, this.AdoNetPrefix))
            {
                if (m_Description.Fields.Count(s => s.Name.Equals("id")) > 0)
                {
                    ...
                    {
                        var usedName = FieldSeparatorL + f.Name + FieldSeparatorR;
                        using (_writer.Block("if(_item.mc_{0})", f.Name))
                        {
                            if(f.Name != "id")
                            {
                                _writer.WriteIndentEol("changed =  true");
                                _writer.WriteIndentEol("cmdText += (cmdText.Length > 0 ? \", \" : \"\") + \"{0} = @{1}\"", usedName, f.Name);
