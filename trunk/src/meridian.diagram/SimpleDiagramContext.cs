...
namespace meridian.diagram
{
    public class SimpleDiagramContext : IDiagramContext
    {
        ...
        public void SyncControllers()
        {
            ...
            var protos = operationContext.GetAllProtos();
            var controllers = operationContext.GetAllControllers();

            foreach (var proto in protos)
            {
                var expectedName = Controller.FromProtoName(proto.Name);

                if (!controllers.Any(s => s.Name.Equals(expectedName)))
                {
                    operationContext.CreateController(expectedName, proto.Name);
                }
            }
